...
- **Colour is never the only signal**: this project is used by people with red-green colour vision deficiency, for whom shades within red or within green are indistinguishable. Any state shown by colour must carry a second, non-colour cue as well - size, shape, icon, position, fill pattern, animation or plain text. A red-vs-green pair with no other marker conveys nothing; where a two-way colour split is genuinely wanted, blue/orange survives the most common deficiencies. This applies to the canvas, the rack view, PDF reports and any chart the app generates.
