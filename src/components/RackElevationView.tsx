...
const RackElevationView: React.FC = () => {
  ...
  const rackableNodes = nodes.filter(n => n.type === 'hardwareNode' || n.data?.configType === 'TAP Device');
  ...
  const rackedNodes = rackableNodes.filter(n => n.data?.rackId === 'rack1' && typeof n.data?.rackU === 'number');
  const unrackedNodes = rackableNodes.filter(n => !n.data?.rackId);
  ...
  const handleDrop = (e: React.DragEvent, uPosition: number) => {
    ...
    if (nodeId) {
      updateNodeData(nodeId, { rackId: 'rack1', rackU: uPosition });
    }
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: '#1e1e1e', color: '#fff', padding: '20px', gap: '20px', boxSizing: 'border-box', overflowY: 'auto' }}>
      ...
      <div style={{ width: '300px', background: '#252526', border: '1px solid #333', borderRadius: '8px', padding: '16px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto', flex: 1 }}>
          {unrackedNodes.length === 0 ? (
            <div style={{ color: '#666', fontSize: '13px', fontStyle: 'italic' }}>All hardware is racked.</div>
          ) : (
        ...
        <div style={{ marginTop: '20px', background: '#111', padding: '16px', borderRadius: '8px', border: '1px solid #333' }}>
          <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', color: '#ffb74d' }}>Rack 1 Metrics</h3>
      ...
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
        <div style={{ width: '400px', background: '#111', border: '10px solid #2d2d2d', borderTop: '20px solid #2d2d2d', borderBottom: '20px solid #2d2d2d', borderRadius: '4px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          {rackUnits.map(u => {
            // Check if any node is mapped to this exact U slot
            const occupyingNode = rackedNodes.find(n => n.data?.rackU === u);
            
            // Check if this slot is covered by a multi-RU device sitting below it
            ...
            if (isCovered && !occupyingNode) {
              return null; // Don't render this slot because a multi-RU device overlaps it
            }
            ...
            if (occupyingNode) {
              ...
              return (
                <div 
                  ...
                  style={{ 
                    height: `${ru * 24}px`, // 1U = 24px visually
                    background: '#007cff', 
                    color: '#fff',
