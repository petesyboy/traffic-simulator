  ...
  return (
    <>
      ...
      <FormGroup label="Site Assignment (Optional)">
        <datalist id="existing-sites-input-list">
          {Array.from(new Set(useStore.getState().nodes.map(n => n.data?.site).filter(s => typeof s === 'string' && (s as string).trim() !== ''))).map(s => (
            <option key={s as string} value={s as string} />
          ))}
        </datalist>
        <input 
          type="text" 
          list="existing-sites-input-list"
