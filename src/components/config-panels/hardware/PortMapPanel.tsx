...
export const PortMapPanel: React.FC<PortMapPanelProps> = ({ selectedNode }) => {
  ...
  if (ports.length === 0 || rows.length === 0) return null;
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(255, 152, 0, 0.2)', paddingTop: '10px', marginTop: '10px' }}>
        ...
        {rows.map((row) => {
          ...
          return (
            <div key={`${row.edgeId}-${row.linkIndex}`} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '10px' }}>
              ...
              {optic && (
                <span style={{ color: '#ff9800', marginLeft: 'auto', flexShrink: 0, fontSize: '9px' }}>
                  {optic.split(' ')[0]}
                </span>
              )}
            </div>
      ...
      <div style={{ fontSize: '9px', color: 'var(--text-secondary)', marginTop: '4px' }}>
        Ports are allocated automatically as links are made. Choosing one here pins it.
      </div>
    </div>
  );
};
