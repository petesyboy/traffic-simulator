...
import opticRulesRaw from './opticRules.json';
...
// SKU -> the exact descriptive string opticRules.json uses for it, so this
// matrix's labels read identically to the HC/TA "Install Optics" picker and
// the TAP "Target Optic" picker (both built from that same catalogue via
// formatOpticLabel() - see nodeTypes.ts). The QSB-* BiDi parts aren't in
// opticRules.json (they're not offered on a normal chassis board install),
// so their descriptive text is hand-written here in the same style.
const SKU_TO_RAW_LABEL: Record<string, string> = {
  'QSB-501': 'QSB-501 (40G QSFP+ BiDi)',
  'QSB-521': 'QSB-521 (100G QSFP28 BiDi)',
  'QSB-523T': 'QSB-523T (100G QSFP28 BiDi)',
  'QSB-531': 'QSB-531 (100G QSFP28 BiDi)',
};
(Object.values(opticRulesRaw) as Record<string, string[]>[]).forEach((boards) =>
  Object.values(boards).forEach((optics) =>
    optics.forEach((raw) => {
      SKU_TO_RAW_LABEL[raw.split(' ')[0]] = raw;
    })
  )
);
